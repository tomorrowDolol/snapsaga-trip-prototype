...
- **固定底栏**：一行最近拍摄（最多 6 + ＋）+ 一行 `[控制][大快门][翻转]`，快门在任何支持的高度下
  都完整可见且不被任何元素覆盖；
...
- 拍摄页只保留取景、快门和必要入口；装饰读数、重复说明和空状态细节收起；
- 主题页移除常驻统计与解释卡，新建主题先显示主题、产出方式和照片；32 个灵感词通过「灵感」展开，布局与强度通过「更多设置」展开；
- 照片页把胶片库收进折叠区，选择照片后才显示拍立得 / 修图等动作；暗房没有任务时不显示 9 个空槽；
- 其它页面统一使用短标题、短按钮和短空状态。功能、数据结构、并发 9 与多图 9 均不变。
...
│   │   └── CameraSheet.tsx    # 取景页的相机抽屉（v0.9：辅助 UI 全在里面；v0.10 加诊断；v0.11 收短文案）
