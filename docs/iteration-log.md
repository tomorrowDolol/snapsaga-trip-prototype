...
- **验证矩阵**（在 `web/` 跑 `npm run verify`）：149 个单测 · 204 项 e2e · 111 项 guard；分层说明见 [`../web/README.md`](../web/README.md)。
...
- 验：**149 单测 / 204 e2e / 111 guard 全绿**（`npm run verify` 退出码 0）；新增 e2e 组「取景页几何与相机抽屉」（65 项，390×844 与 390×664 两个视口各跑一遍：快门在视口内且与任何其它可见元素不相交、取景画面 ≥ 视口 60%、页面与视图都不滚动、抽屉关闭时取景画面里没有常驻辅助块、抽屉开关与下拉手势、抽屉开着也能按快门）；新增 guard 18 项（辅助 UI 在抽屉里、`#shutter` 唯一且在 dock 里、`#view-cam` 不滚动、AppHeader 隐藏、调试桥抽屉开关…）；`git diff --stat main -- index.html` 无改动（根原型仍 v0.6）。遗留：新增 K29（构图提示单行会截断）。
