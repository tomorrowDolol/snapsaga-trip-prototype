...
| e2e | **204** | 主链路 36 · 主题模式 63 · **取景页几何与相机抽屉 65（新）** · 缩略图 13 · 拍照三环境 17 · AI Base 10 |
| guard | **111** | 源码侧 67 / 产物侧 25 / 产物运行时 12 / 子路径部署冒烟 7 |
...
新增 guard 18 项（源码侧 15 / 产物侧 3）：辅助 UI 全部在 `CameraSheet` 里且 `CameraView` 不再渲染它们、
...
AppHeader 在取景页不渲染它们、构图提示单行 + 3.5 s 定时、抽屉是绝对定位层、把手有指针手势、调试桥有抽屉开关；
